...
class ModelSaleOrder extends Model {
    public function getOrder($order_id) {
        ...
        if ($order_query->num_rows) {
            ...
            $customer_info = $this->model_customer_customer->getCustomer($order_query->row['customer_id']);
      ...
      'tracking_carrier' => isset ($order_query->row['tracking_carrier']) ? $order_query->row['tracking_carrier'] : '',
      
                'tckn'                    => $customer_info['tckn'],
                'class_id'                => $customer_info['class_id'],
    ...
    public function getOrders($data = array()) {
      ...
      $qosu_query = '';
      if ($this->config->get('qosu_order_statuses')) {
        $qosu_query = ' o.order_status_id, o.tracking_no, o.tracking_url,';
      } else {
        $qosu_query = ' o.order_status_id,';
      }
      
        $sql = "SELECT  o.*, o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS order_status, o.shipping_code, o.total, o.currency_code, o.currency_value, o.date_added, ".$qosu_query." o.date_modified FROM `" . DB_PREFIX . "order` o";
        ...
        if (!empty($data['filter_payment_type'])) {
            $sql .= " AND o.payment_code = '" . $this->db->escape($data['filter_payment_type']) . "'";
        }
        ...
        if (!empty($data['filter_store_id'])) {
            ...
            if (isset($data['filter_stores']))
                $sql .= " AND o.store_id IN (" . $data['filter_stores'] . ")";
        }
        ...
        if (!empty($data['filter_class'])) {
            $sql .= " AND o.customer_id IN (SELECT customer_id FROM customer WHERE class_id IN (SELECT class_id FROM class WHERE name LIKE '%" . $data['filter_class'] . "%'))";
        }

        if (!empty($data['filter_date_added']) && !empty($data['filter_date_end'])) {
        ...
        } else if (!empty($data['filter_date_added'])) {
    ...
    public function getTotalOrders($data = array()) {
        ...
        if (!empty($data['filter_store_id'])) {
            ...
            if (isset($data['filter_stores']))
                $sql .= " AND store_id IN (" . $data['filter_stores'] . ")";
        }
        ...
        if (!empty($data['filter_payment_type'])) {
            $sql .= " AND payment_code = '" . $this->db->escape($data['filter_payment_type']) . "'";
        }
    ...
    public function getOrderHistories($order_id, $start = 0, $limit = 10) {
        ...
        $query = $this->db->query("SELECT oh.date_added, os.name AS status, oh.comment, oh.notify, oh.order_history_id FROM " . DB_PREFIX . "order_history oh LEFT JOIN " . DB_PREFIX . "order_status os ON oh.order_status_id = os.order_status_id WHERE oh.order_id = '" . (int)$order_id . "' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY oh.date_added DESC LIMIT " . (int)$start . "," . (int)$limit);
