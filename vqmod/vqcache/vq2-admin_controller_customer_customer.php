...
class ControllerCustomerCustomer extends Controller {
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if (isset($this->request->post['ajax'])) {
                echo json_encode(array('success' => 'success'), JSON_UNESCAPED_UNICODE);
                exit();
            }
    ...
    protected function getList() {
        ...
        if (isset($this->request->get['filter_class_id'])) {
            $filter_class_id = $this->request->get['filter_class_id'];
        } else {
            $filter_class_id = '';
        }
        ...
        if (isset($this->request->get['filter_class_id'])) {
            $url .= '&filter_class_id=' . $this->request->get['filter_class_id'];
        }
        ...
        $data['classes'] = $this->model_catalog_class->getClasses();
        ...
        $data['sort_class_id'] = $this->url->link('customer/customer', 'user_token=' . $this->session->data['user_token'] . '&sort=c.class_id' . $url, true);
        ...
        $data['filter_class_id'] = $filter_class_id;
    ...
    protected function getForm() {
        ...
        $classes = $this->model_catalog_class->getClasses();
        ...
        if (isset($this->request->post['class_id'])) {
            ...
            $data["class_id"] = $customer_info['class_id'];
        } else {
            $data["class_id"] = '';
        }
        ...
        if (isset($this->request->post['tckn'])) {
            $data['tckn'] = $this->request->post['tckn'];
        } elseif (!empty($customer_info)) {
            $data['tckn'] = $customer_info['tckn'];
        } else {
            $data['tckn'] = '';
        }

        if (isset($this->request->post['stdn'])) {
            $data['stdn'] = $this->request->post['stdn'];
        } elseif (!empty($customer_info)) {
            $data['stdn'] = $customer_info['stdn'];
        } else {
            $data['stdn'] = '';
        }
    ...
    protected function validateForm() {
        ...
        if ($this->request->post['password'] || (!isset($this->request->get['customer_id']))) {
            if (strlen($this->request->post['password']) === 0 && (!isset($this->request->get['customer_id']))) {
                $this->load->model('customer/customer_group');
                $customer_group = $this->model_customer_customer_group->getCustomerGroup($this->request->post['customer_group_id']);
                $this->request->post['password'] = $this->request->post['confirm'] = $customer_group['default_password'];
            }
    ...
    public function autocomplete() {
        ...
        if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email'])) {
            ...
            foreach ($results as $result) {
                ...
                if ($result['class_id']) {
                    $products = $this->model_customer_customer->getCustomerProducts($result['class_id']);
                    foreach ($products as $product_id) {
                        $product = $this->model_catalog_product->getProduct($product_id['product_id']);
                        $product['package_id'] = $product_id['package_id'];
                        $product['package_quantity'] = $product_id['package_quantity'];
                        $product['price'] = number_format((float)$product['price'], 2, '.', '') . "₺";
                        $product['package_price'] = $product_id['package_price'] > 0 ? number_format((float)$product_id['package_price'], 2, '.', '') . "₺" : number_format((float)$product['price'], 2, '.', '') . "₺";
                        $package_products[] = $product;
                    }

                    $class = $this->model_catalog_class->getClass($result['class_id']);
                }
                ...
                $json[] = array(
                    ...
                    'class'             => $class,
                    'class_id'          => $result['class_id'] ? $result['class_id'] : 0
                );
            }
        }
