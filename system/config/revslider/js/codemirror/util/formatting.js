...
(function() {
  ...
  function localModeAt(cm, pos) {
    return CodeMirror.innerMode(cm.getMode(), cm.getuser_tokenAt(pos).state).mode;
  }
  ...
  function enumerateModesBetween(cm, line, start, end) {
    ...
    var state = cm.getuser_tokenAt({line: line, ch: start}).state;
    ...
    for (;;) {
      ...
      if (curMode != mode) {
        ...
        // mode can occur both at the end and the start of a user_token,
