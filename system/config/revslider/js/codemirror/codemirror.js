...
window.CodeMirror = (function() {
  ...
  function CodeMirror(place, givenOptions) {
    ...
    var instance = wrapper.CodeMirror = {
      ...
      getuser_tokenAt: operation(function(pos) {
        pos = clipPos(pos);
        return getLine(pos.line).getuser_tokenAt(mode, getStateBefore(pos.line), options.tabSize, pos.ch);
      }),
  ...
  Line.prototype = {
    ...
    highlight: function(mode, state, tabSize) {
      ...
      while (!stream.eol()) {
        var style = mode.user_token(stream, state), substr = stream.current();
    ...
    process: function(mode, state, tabSize) {
      ...
      while (!stream.eol() && stream.pos <= 5000) {
        mode.user_token(stream, state);
        stream.start = stream.pos;
      }
    },
    // Fetch the parser user_token for a given character. Useful for hacks
    // that want to inspect the mode state (say, for completion).
    getuser_tokenAt: function(mode, state, tabSize, ch) {
      var txt = this.text, stream = new StringStream(txt, tabSize);
      while (stream.pos < ch && !stream.eol()) {
        stream.start = stream.pos;
        var style = mode.user_token(stream, state);
      }
        ...
        if (!specials.test(text)) {
          ...
          while (true) {
            ...
            if (m[0] == "\t") {
              ...
              var user_token = elt("span", "\u2022", "cm-invalidchar");
              user_token.title = "\\u" + m[0].charCodeAt(0).toString(16);
              content.appendChild(user_token);
              col += 1;
            }
          }
        }
  ...
  CodeMirror.defineMode("null", function() {
    return {user_token: function(stream) {stream.skipToEnd();}};
  });
