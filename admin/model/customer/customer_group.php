...
class ModelCustomerCustomerGroup extends Model {
    public function addCustomerGroup($data) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "customer_group SET approval = '" . (int)$data['approval'] . "', sort_order = '" . (int)$data['sort_order'] . "', default_password = '" . $this->db->escape($data['default_password']) . "'");
    ...
    public function editCustomerGroup($customer_group_id, $data) {
        $this->db->query("UPDATE " . DB_PREFIX . "customer_group SET approval = '" . (int)$data['approval'] . "', sort_order = '" . (int)$data['sort_order'] . "', default_password = '" . $this->db->escape($data['default_password']) . "' WHERE customer_group_id = '" . (int)$customer_group_id . "'");
        ...
        $this->db->query("UPDATE " . DB_PREFIX . "customer SET salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['default_password'])))) . "', code = '' WHERE customer_group_id = '" . (int)$customer_group_id . "'");
    }
