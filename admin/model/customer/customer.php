...
class ModelCustomerCustomer extends Model {
    public function addCustomer($data) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', tckn = '" . $this->db->escape($data['tckn']) . "', stdn = '" . $this->db->escape($data['stdn']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode(array())) . "', newsletter = '" . (int)$data['newsletter'] . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', status = '" . (int)$data['status'] . "', safe = '" . (int)$data['safe'] . "', date_added = NOW(), class_id = " . (int)$data['class_id']);
    ...
    public function editCustomer($customer_id, $data) {
        $this->db->query("UPDATE " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', tckn = '" . $this->db->escape($data['tckn']) . "', stdn = '" . $this->db->escape($data['stdn']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode(array())) . "', newsletter = '" . (int)$data['newsletter'] . "', status = '" . (int)$data['status'] . "', safe = '" . (int)$data['safe'] . "', class_id = " . (int)$data['class_id'] . " WHERE customer_id = '" . (int)$customer_id . "'");
    ...
    public function getCustomers($data = array()) {
        ...
        if (!empty($data['filter_name'])) {
            $implode[] = "(CONCAT(c.firstname, ' ', c.lastname) LIKE '%" . $this->db->escape($data['filter_name']) . "%' OR tckn LIKE '%" . $this->db->escape($data['filter_name']) . "%' OR stdn LIKE '%" . $this->db->escape($data['filter_name']) . "%')";
        }
        ...
        if (!empty($data['filter_class_id'])) {
            $implode[] = "c.class_id = '" . (int)$data['filter_class_id'] . "'";
        }
    ...
    public function getTotalCustomers($data = array()) {
        ...
        if (!empty($data['filter_class_id'])) {
            $implode[] = "class_id = '" . (int)$data['filter_class_id'] . "'";
        }
    ...
    public function getCustomerProducts($class_id) {
        $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "class_package WHERE class_id = " . (int)$class_id);
        $product_ids = [];
        foreach ($query->rows as $package) {
            $product_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "package_product WHERE package_id = " . (int)$package['package_id'] . " ORDER BY sort_order");
            foreach ($product_query->rows as $product) {
                $product_ids[$product['product_id']] = array('package_id' => $package['package_id'], 'product_id' => $product['product_id'], 'package_quantity' => $product['quantity'], "package_price" => $product['package_price']);
            }
        }
