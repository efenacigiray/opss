...
class ControllerCatalogProduct extends Controller {
    ...
    protected function getForm() {
        ...
        if (isset($this->request->post['sku'])) {
            $data['sku'] = str_replace("'", "", $this->request->post['sku']);
        } elseif (!empty($product_info)) {
            $data['sku'] = str_replace("'", "", $product_info['sku']);
        } else {
            $data['sku'] = '';
        }
        ...
        $data['store_stocks'] = array();
        if (isset($this->request->post['store_stocks'])) {
            $data['store_stocks'] = $this->request->post['store_stocks'];
        } elseif (isset($this->request->get['product_id'])) {
            $data['store_stocks'] = $this->model_catalog_product->getProductStoreStocks($this->request->get['product_id']);
            if (empty($data['store_stocks'])) {
                foreach ($stores as $store) {
                    $data['store_stocks'][] = array(
                        'store_id' => $store['store_id'],
                        'name'     => mb_convert_case(mb_strtolower($store['name'], 'utf8'), MB_CASE_TITLE, 'utf8'),
                        'quantity' => null
                    );
                }
            } else {
                foreach ($data['store_stocks'] as &$value) {
                    $value['name'] = mb_convert_case(mb_strtolower($value['name'], 'utf8'), MB_CASE_TITLE, 'utf8');
                }
            }
        } else {
            foreach ($stores as $store) {
        ...
        if (isset($this->request->post['price'])) {
            ...
            $data['price'] = number_format((float)$product_info['price'], 2, '.', '');
        } else {
            $data['price'] = '';
        }
        ...
        $data['taxed_price'] = $data['price'];
        if (!empty($product_info)) {
            $rules = $this->model_localisation_tax_class->getTaxRules($product_info['tax_class_id']);
            foreach ($rules as $tax_rule) {
                $tax_rate = $this->model_localisation_tax_rate->getTaxRate($tax_rule['tax_rate_id']);
                $data['taxed_price'] = round(($data['taxed_price'] * $tax_rate['rate'] / 100) + $data['taxed_price'], 2);
                $data['taxed_price'] = number_format((float)$data['taxed_price'], 2, '.', '');
            }
        }
    ...
    protected function validateForm() {
        ...
        if ((utf8_strlen($this->request->post['model']) < 1) || (utf8_strlen($this->request->post['model']) > 64)) {
            ...
            for ($i = 0; $i < 12; $i++) {
                $this->request->post['model'] .= mt_rand(0, 9);
            }
        }
    ...
    public function autocomplete() {
        ...
        if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model'])) {
            ...
            foreach ($results as $result) {
                ...
                $manu_name = isset($manufacturer['name']) && !empty($manufacturer['name'])? $manufacturer['name'] . " - " : "";

                $name = strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8'));
                if (isset($this->request->get['add_manu_name'])) {
                    $name = strip_tags(html_entity_decode($manu_name . $result['name'] . " - " . $result['model'], ENT_QUOTES, 'UTF-8'));
                }
    ...
    public function getStoreSales() {
        ...
        $date_start = $this->request->get['date_start'] ? $this->request->get['date_start'] : date('Y-m-01 00:00:00');
        $date_end = $this->request->get['date_end'] ? $this->request->get['date_end'] : date('Y-m-d 23:59:59');

        $this->load->model('extension/dashboard/store_sales');
        $orders = $this->model_extension_dashboard_store_sales->getStoreSales($store_id, $date_start, $date_end);
        $data['total'] = 0;
        $data['total_orders'] = count($orders);
        $data['total_with_type'] = [];

        foreach ($orders as $order) {
            $data['total'] += $order['total'];
            if (!isset($data['total_with_type'][$order['payment_method']])) {
                $data['total_with_type'][$order['payment_method']] = ['count' => 0, 'total' => 0];
            }
            $data['total_with_type'][$order['payment_method']]['count']++;
            $data['total_with_type'][$order['payment_method']]['total'] += $order['total'];
        }


        foreach ($data['total_with_type'] as $key => &$value) {
            $value['total'] = $this->currency->format($value['total'], $this->config->get('config_currency'));
        }

        $data['total'] = $this->currency->format($data['total'], $this->config->get('config_currency'));

        $grouped_orders = $this->model_extension_dashboard_store_sales->getGroupedStoreSales($store_id, $date_start, $date_end);
        $data['total_package_orders'] = count($grouped_orders);
        $data['package_orders'] = [];
        foreach ($grouped_orders as $grouped_order) {
            if (!isset($data['package_orders'][$grouped_order['class']])) {
                if (!$grouped_order['class']) continue;
                $data['package_orders'][$grouped_order['class']] = ['order' => 0, 'students' => $this->model_extension_dashboard_store_sales->getStudentTotal($grouped_order['class_id_o'])];
            }
            $data['package_orders'][$grouped_order['class']]['order']++;
        }

        ksort($data['package_orders']);
