<?php
class ControllerSaleStoreOrder extends Controller {
    private $error = array();
    ...
    public function edit() {
        ...
        $store_statuses = $this->config->get('config_store_status');
        ...
        $data['tckn'] = $order_info['tckn'];
        $data['class_id'] = $order_info['class_id'];
        $data['class'] = "";
        foreach ($data['classes'] as $value) {
            if ($value['class_id'] == $data['class_id'])
                $data['class'] = $value['name'];
        }
        ...
        foreach ($data['order_vouchers'] as &$value) {
            $value['formatted'] = $this->currency->format($value['amount'], $order_info['currency_code'], $order_info['currency_value']);
        }
        $data['coupon'] = '';
        $data['voucher'] = '';
        $data['reward'] = '';
        ...
        foreach ($results as $result) {
            if (in_array($result['store_id'], $this->session->data['stores'])) {
                $data['stores'][] = array(
        ...
        if (in_array(0, $this->session->data['stores'])) {
            $data['stores'][] = array(
                'store_id' => 0,
        ...
        $store_statuses = array_reverse($store_statuses);

        foreach ($data['order_statuses'] as $key => &$value) {
            if (in_array($value['order_status_id'], $store_statuses)) {
                unset($data['order_statuses'][$key]);
                array_unshift($data['order_statuses'], $value);
                unset($value);
            }
        }
        ...
        if ($api_info && $this->user->hasPermission('modify', 'sale/store_order')) {
        ...
        if (in_array('customer/quick_add', $this->user->getPermission()['access'])) {
            $data['quick_add'] = true;
        }
        ...
        $this->response->setOutput($this->load->view('sale/store_order_update_form', $data));
    ...
    public function getForm() {
        ...
        if (empty($store_statuses))
            $data['order_status_id'] = $this->config->get('config_order_status_id');
        else
            $data['order_status_id'] = $store_statuses[0];
        ...
        $this->response->setOutput($this->load->view('sale/store_order_form', $data));
    }
    ...
    protected function validate() {
        if (!$this->user->hasPermission('modify', 'sale/store_order')) {
