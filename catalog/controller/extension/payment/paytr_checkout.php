...
class ControllerExtensionPaymentPaytrCheckout extends Controller {
    ...
    public function gettoken()
    {
        ...
        $post_vals      = array(
            ...
            'currency'          => $currency,
            'merchant_ok_url'   => $this->getSiteUrl() .'index.php?route=checkout/success&order_id=' . $order_info['order_id'],
    ...
    {
        if (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) { $siteUrl = $this->config->get('config_ssl'); }
        else { $siteUrl = $this->config->get('config_url'); }
	...
	public function callback()
    {
        ...
        if ( $getOrder ) {
            ...
            } else if ( $_POST['status'] == 'failed' AND array_key_exists( 'failed_reason_code', $_POST ) AND $_POST['failed_reason_code'] != 6 ) {
