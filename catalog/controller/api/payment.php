...
class ControllerApiPayment extends Controller {
    ...
    public function methods() {
        ...
        if (!isset($this->session->data['api_id'])) {
            ...
            if (isset($this->request->get['ignore_address']) && $this->request->get['ignore_address'] == 'true') {
                $json = false;
                $this->session->data['payment_address'] = array(
                    "country_id" => 215,
                    "zone_id" => 0
    ...
    public function method() {
        ...
        if (!isset($this->session->data['api_id'])) {
            ...
            if ($this->request->get['ignore_address'] == 'true') {
                $json = false;
                $this->session->data['payment_address'] = array(
                    "country_id" => 215,
                    "zone_id" => 0
            ...
            foreach ($results as $result) {
                if ($this->config->get('payment_' . $result['code'] . '_status')) {
                    ...
                    if ($method) {
                        $methods['payment_methods'][$result['code']] = $method;
                    }
                }
            }

            $this->session->data['payment_methods'] = $methods['payment_methods'];
