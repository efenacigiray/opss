...
class ModelAccountOrder extends Model {
    ...
    public function getOrders($start = 0, $limit = 20, $package = false) {
        if ($start < 0) {
            $start = 0;
        }
        ...
        if ($package) {
            $order_statuses = array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'));
            $implode = "IN (" . implode(',', $order_statuses) .")";
        } else {
            $implode = " > 0";
        }
